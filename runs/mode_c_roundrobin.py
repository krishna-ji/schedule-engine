...
NSGA-II + Round-Robin Repair - Cycles through repair heuristics in fixed order.
...
def setup_logging(output_dir: Path) -> logging.Logger:
    ...
    log_file = output_dir / "mode_c_roundrobin.log"
...
def main() -> None:
    ...
    OUTPUT_DIR = PROJECT_ROOT / "output" / "mode_c_roundrobin" / TIMESTAMP
    ...
    plot_convergence(
        stats, OUTPUT_DIR / "mode_c_convergence.png", title_prefix="Mode C: "
    )
    logger.info(f"Saved: {OUTPUT_DIR / 'mode_c_convergence.png'}")

    plot_constraint_breakdown(
        breakdown,
        OUTPUT_DIR / "mode_c_breakdown.png",
        title="Mode C: Constraint Violations",
    )
    logger.info(f"Saved: {OUTPUT_DIR / 'mode_c_breakdown.png'}")
    ...
    export_paths = export_full_results(
        ...
        mode_name="mode_c_roundrobin",
    )

    metadata = {
        "experiment": "mode_c_roundrobin",
        "timestamp": TIMESTAMP,
        "config": {
            "seed": SEED,
            "pop_size": POP_SIZE,
