...
if overloaded_groups:
    ...
    for gid, gname, dem, avail, util in sorted(overloaded_groups, key=lambda x: -x[4]):
...
for raw_i in raw_instructors:
    ...
    if not avail:
        continue
    for day, periods in avail.items():
        if not periods:
            continue
...
unique_missing_codes = set(cc for _, cc in group_course_issues)
...
# F5 – Duplicate IDs
dup_instructors = len(raw_instructors) - len(set(i["id"] for i in raw_instructors))
dup_rooms = len(raw_rooms) - len(set(r["room_id"] for r in raw_rooms))
dup_courses = len(raw_courses) - len(set(c["CourseCode"] for c in raw_courses))
...
with open(report_path, "w") as f:
    ...
    f.write(f"Generated by: runs/pre_scheduling_audit.py\n")
    f.write("=" * 80 + "\n\n")
