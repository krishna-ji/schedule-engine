...
def run_ppo_eval(
    ...
    for step in range(1, EVAL_MAX_GEN + 1):
        ...
        if verbose:
            ...
            print(
                f"    {step:>4} │ {best_hard:>5.0f} {best_soft:>5.0f} │ {name:>12} │ {prob_str} │ {reward:>7.3f}"
            )
    ...
    best_hard_gen = (
        min(range(len(rows)), key=lambda i: rows[i]["best_hard"]) if rows else 0
    )
    ...
    if verbose:
        print(
            f"    Final: hard={final['best_hard']:.0f}, soft={final['best_soft']:.0f}"
        )
        print(
            f"    Best hard: {best_hard_val:.0f} at gen {best_hard_gen + 2} (soft={soft_at_best:.0f})"
        )
        print(f"    Cumulative R: {cumulative_reward:.3f}")
        print(f"    Prob variance: {result['prob_variance']:.6f}")
        freq_str = ", ".join(
            f"{ACTION_SHORT[a]}={action_freq.get(a, 0)}" for a in range(6)
        )
...
def main() -> None:
    ...
    r = det_result
    freq_str = ", ".join(
        f"{ACTION_SHORT[a]}={r['action_freq'].get(a, 0)}"
        for a in range(6)
        if r["action_freq"].get(a, 0) > 0
    )
    print(
        f"  {'PPO Determ (seed=42)':<28s} {r['final_hard']:>8.0f} {r['final_soft']:>8.0f} "
    ...
    for r in stoch_results:
        ...
        print(
            f"  {'PPO Stoch (seed=' + str(r['seed']) + ')':<28s} {r['final_hard']:>8.0f} {r['final_soft']:>8.0f} "
            f"{r['best_hard']:>6.0f} {r['soft_at_best_hard']:>8.0f} {freq_str:>30s}"
        )
    ...
    for aid in range(6):
        bl = baselines[aid]
        print(
            f"  Static {aid} ({bl['name']:<14s}) {bl['final_hard']:>8.0f} {bl['final_soft']:>8.0f} "
    ...
    print(
        f"    Stochastic:    best_hard={mean_stoch_h:.1f}±{np.std(stoch_hards):.1f}, "
        f"soft={mean_stoch_s:.1f}±{np.std(stoch_softs):.1f}"
    )
    if mean_stoch_h < det_h:
        print(
            f"    >>> STOCHASTIC IS BETTER by {det_h - mean_stoch_h:.1f} hard points!"
        )
    elif mean_stoch_h > det_h:
        print(
            f"    >>> DETERMINISTIC IS BETTER by {mean_stoch_h - det_h:.1f} hard points"
        )
    else:
        print("    >>> TIE on hard — check soft constraints")
    ...
    best_stoch = min(
        stoch_results, key=lambda r: (r["best_hard"], r["soft_at_best_hard"])
    )
    best_static = min(
        baselines.values(), key=lambda b: (b["best_hard"], b["soft_at_best_hard"])
    )
    print(
        f"    Best stochastic: hard={best_stoch['best_hard']:.0f}, soft={best_stoch['soft_at_best_hard']:.0f}"
    )
    print(
        f"    Best static:     hard={best_static['best_hard']:.0f}, soft={best_static['soft_at_best_hard']:.0f} ({best_static['name']})"
    )
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for aid, bl in baselines.items():
            writer.writerow(
                [
                    f"Static_{aid}_{bl['name']}",
                    EVAL_SEED,
                    True,
                    bl["final_hard"],
                    bl["final_soft"],
                    bl["best_hard"],
                    bl["soft_at_best_hard"],
                    0,
                    0,
                ]
            )
...
def _generate_plot(
    ...
    for aid in range(6):
        ...
        ax.plot(
            ...
            linewidth=1.0,
            linestyle="--",
            label=f"Static {ACTION_SHORT[aid]}",
        )
    ...
    for i, sr in enumerate(stoch_results):
        ...
        ax.plot(
            s_gens,
            s_hard,
            color=stoch_colors[i % len(stoch_colors)],
            linewidth=1.5,
            alpha=0.8,
            label=f"PPO Stoch (seed={sr['seed']})",
            zorder=9,
        )
    ...
    ax2.bar(
        action_gens,
        [1] * len(best_stoch["actions_taken"]),
    ...
    ax2.set_yticks([])
    ax2.set_title(
        f"Best Stochastic Run Action Sequence (seed={best_stoch['seed']})", fontsize=10
    )
    legend_elements = [
        Patch(facecolor=colors[a], label=f"{a}: {ACTION_NAMES[a]}") for a in range(6)
    ]
