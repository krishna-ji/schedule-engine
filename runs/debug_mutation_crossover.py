...
for trial in range(10):
    ...
    print(f"  Trial {trial}: {orig_fitness[0]:.0f} -> {new_fitness[0]:.0f} ({status}, delta={change:+.0f})")
...
for trial in range(10):
    ...
    print(f"  Trial {trial}: best({orig_best_fitness:.0f}) x partner({partner_fit:.0f}) -> offspring({new_fit1:.0f}, {new_fit2:.0f}) best={best_offspring:.0f} ({status})")
