...
Phase 5 of the OOP redesign. Unifies:
- ``ga/population.py`` (20 free functions, 1365 lines)
- ``ga/hybrid_population.py`` (10 free functions, 496 lines)
- ``ga/run_helpers.py::create_random_individual()``

All three files are preserved — this is a facade that delegates to
the existing implementations, providing a single API surface for
population creation.  Over time, the internal helpers can be migrated
to private methods.
...
class PopulationFactory:
    ...
    def greedy_individual(self) -> list[SessionGene]:
        ...
        Uses ``hybrid_population._greedy_construction`` which places
        the hardest-to-schedule courses first.
        """
    ...
    def _hybrid_population(self, n: int) -> list[list[SessionGene]]:
        """Mix greedy + random (delegates to hybrid_population.py)."""
        from schedule_engine.ga.hybrid_population import generate_hybrid_population
