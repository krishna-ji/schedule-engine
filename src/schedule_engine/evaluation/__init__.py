...
Phase 3 of the OOP redesign. Replaces:
- ``ga/evaluator/fitness.py`` (evaluate + evaluate_from_timetable)
- ``ga/evaluator/detailed_fitness.py`` (evaluate_detailed + evaluate_from_detailed)
- ``ga/run_helpers.py::create_evaluator()`` (closure-based evaluator)
- ``ga/run_helpers.py::get_constraint_breakdown()``
- ``constraints/all_constraints.py`` (evaluate_all, evaluate_hard_constraints,
  evaluate_soft_constraints)

All seven implementations become one class with two methods.

Usage::

    from schedule_engine.evaluation.evaluator import Evaluator
