...
from schedule_engine.constraints.soft import paired_cohort_practical_alignment, break_placement_compliance
...
def main() -> None:
    ...
    from schedule_engine.constraints.soft import _build_group_day_schedules, _get_break_window_quanta
    ...
    for (group_id, day_name), occupied_quanta in group_schedules.items():
        ...
        if free_in_break < min_free:
            ...
            if violation_count <= 5:  # Show first 5 violations
                print(f"   Violation {violation_count}: {group_id} on {day_name}: {shortage} quanta short = {penalty} penalty")
    
    print(f"   Total violations: {violation_count}")
    print(f"   Total penalty: {total_violations}")
    print(f"   ➜ Penalty per quantum shortage: {break_penalty}")
    print(f"   ➜ RECOMMENDATION: Reduce break_violation_penalty from {break_penalty} to 1-10")
