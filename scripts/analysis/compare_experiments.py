...
def load_experiment_results() -> List[Dict[str, Any]]:
    ...
    if not experiments_dir.exists():
        console.print("[red]No experiments directory found at output/experiments/[/red]")
        console.print("[yellow]Run 'uv run migrate' to organize existing experiments[/yellow]")
        return []
        ...
        if metrics_file.exists():
            try:
                with open(metrics_file) as f:
                    ...
                    if len(path_parts) >= 4:
                        data["method_category"] = path_parts[-3]  # baseline, repairs, etc.
                        data["method_type"] = path_parts[-2]     # pure-nsga, nsga-repairs, etc.
                    experiments.append(data)
            except Exception as e:
                console.print(f"[yellow]Warning: Could not load {metrics_file}: {e}[/yellow]")
    
    console.print(f"[green]Loaded {len(experiments)} experiments from output/experiments/[/green]")
    return experiments
...
def generate_comparison_table(experiments: List[Dict[str, Any]]) -> Table:
    ...
    for exp in sorted_experiments:
        ...
        table.add_row(
            method,
            group,
            f"{hard_violations:.0f}" if isinstance(hard_violations, (int, float)) else str(hard_violations),
            f"{soft_penalty:.2f}" if isinstance(soft_penalty, (int, float)) else str(soft_penalty),
            f"{runtime_hours:.1f}",
            f"{hypervolume:.3f}" if isinstance(hypervolume, (int, float)) else str(hypervolume),
...
def plot_convergence_comparison(experiments: List[Dict[str, Any]]) -> None:
    ...
    for exp in experiments:
        ...
        if hard_violations and soft_penalties:
            ...
            ax1.plot(generations, hard_violations, 
                    label=name[:20], color=colors[color_idx % len(colors)], 
                    linewidth=2, alpha=0.8)
            
            # Plot soft penalties  
            ax2.plot(generations, soft_penalties,
                    label=name[:20], color=colors[color_idx % len(colors)],
                    linewidth=2, alpha=0.8)
    ...
    console.print(f"[green]Convergence comparison plot saved to {output_path}/convergence_comparison.png[/green]")
...
def plot_runtime_vs_quality(experiments: List[Dict[str, Any]]) -> None:
    ...
    scatter1 = ax1.scatter(runtimes, hard_violations, s=100, alpha=0.7, c=range(len(methods)))
    ...
    for i, method in enumerate(methods):
        ax1.annotate(method, (runtimes[i], hard_violations[i]), 
                    xytext=(5, 5), textcoords='offset points', fontsize=8)
    
    # Runtime vs Soft Penalties
    scatter2 = ax2.scatter(runtimes, soft_penalties, s=100, alpha=0.7, c=range(len(methods)))
    ...
    # Add method labels
    for i, method in enumerate(methods):
        ax2.annotate(method, (runtimes[i], soft_penalties[i]),
                    xytext=(5, 5), textcoords='offset points', fontsize=8)
    ...
    console.print(f"[green]Runtime vs quality plot saved to {output_path}/runtime_vs_quality.png[/green]")
...
def generate_statistical_analysis(experiments: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    stats = {
        ...
        "best_hypervolume": df.loc[df["hypervolume"].idxmax()]["method"] if df["hypervolume"].max() > 0 else "N/A",
        "correlation": df.corr(),
    }
...
def main():
    ...
    if not experiments:
        ...
        console.print("[dim]Run experiments first: scripts/run_thesis_experiments.ps1[/dim]")
        return
