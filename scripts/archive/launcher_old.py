...
def run_experiment_universal(
    ...
    **overrides: Any,
    ...
    """
    Universal experiment runner using new config system.

    This function replaces all individual main_* functions.
    It loads experiments from configs/experiments.py and applies
    profiles + overrides dynamically.

    Args:
        experiment_id: Experiment identifier (baseline, memetic, rl-guided, etc.)
        profile: Profile name (test, prod, debug, custom)
        name: Optional experiment run name
        **overrides: Dynamic parameter overrides (ngen=100, cxpb=0.8, etc.)

    Returns:
        Exit code (0 = success)
    """

    from configs.experiment_registry import get_experiment, get_experiment_metadata
    ...
    config = experiment.with_profile(resolved_profile.value, **overrides)

    # Initialize global config
    init_config(config)

    # Display experiment info
    metadata = get_experiment_metadata(experiment_id)
    console.print(
        f"[green]{metadata['name']} ({_profile_banner(resolved_profile)})[/green]"
    )
    console.print(f"[dim]  {metadata['description']}[/dim]")
    if metadata["notes"]:
        console.print(f"[dim]  {metadata['notes']}[/dim]")

    # Display overrides if any
    if overrides:
        override_str = ", ".join(f"{k}={v}" for k, v in overrides.items())
        console.print(f"[yellow]Overrides: {override_str}[/yellow]")

    # Create output directory using ExperimentManager
    ...
    exp_name = (
        sanitize_experiment_name(name)
        if name
        else f"{metadata['id']}_{resolved_profile.value}"
    )

    output_dir = manager.create_output_dir(
        runtime_mode=experiment_id,
    ...
    experiment_run = manager.register_run(
        runtime_mode=experiment_id,
        config_reference=f"{metadata['name']}:{resolved_profile.value}",
    ...
    try:
        ...
        if result["best_individual"] is None:
            console.print("[yellow]Warning: No valid solution found[/yellow]")
            return 1
...
def main_train_rl():
    ...
    if args.curriculum:
        ...
        timestep_map = {"test": 10_000, "prod": 300_000}
...
def main_heuristic_testing():
    ...
    return run_experiment_universal(
        experiment_id="heuristic-testing",
        profile=args.profile,
        name=args.name,
    )
