...
class TestPhase3ExportPDFFlag:
    ...
    def test_export_pdf_false_accepted(self):
        ...
        parent_sig = inspect.signature(BaselineExperiment.__mro__[1].__init__)
...
class TestPhase5CallbackMOEALists:
    ...
    def test_baseline_callback_has_moea_lists(self):
        """_make_progress_cb should produce callback with MOEA lists."""
        from src.experiments.ga_experiment import _make_progress_cb

        cb = _make_progress_cb(log_interval=10)
        assert hasattr(cb, "hypervolumes")
        assert hasattr(cb, "spacings")
        assert hasattr(cb, "diversities")
        assert hasattr(cb, "feasibility_rates")
        assert isinstance(cb.hypervolumes, list)
        assert isinstance(cb.spacings, list)
        assert isinstance(cb.diversities, list)
        assert isinstance(cb.feasibility_rates, list)

    def test_init_moea_lists(self):
        """_init_moea_lists should attach empty lists."""
        ...
        d = Dummy()
        _init_moea_lists(d)
        assert d.hypervolumes == []
        assert d.spacings == []
        assert d.diversities == []
        assert d.feasibility_rates == []

    def test_record_moea_metrics_interval(self):
        """_record_moea_metrics should only record every _METRICS_INTERVAL gens."""
        from src.experiments.ga_experiment import (
            _METRICS_INTERVAL,
            _init_moea_lists,
        ...
        F = np.array([[1.0, 10.0], [5.0, 5.0], [10.0, 1.0]])
        G = np.zeros((3, 8))

        # Not at interval — should NOT record
        alg = FakeAlgorithm(n_gen=3)
        _record_moea_metrics(cb, alg, F, G)
        assert len(cb.hypervolumes) == 0

        # At interval — SHOULD record
        alg = FakeAlgorithm(n_gen=_METRICS_INTERVAL)
        _record_moea_metrics(cb, alg, F, G)
        assert len(cb.hypervolumes) == 1
        assert len(cb.spacings) == 1
        assert len(cb.diversities) == 1
        assert len(cb.feasibility_rates) == 1
        assert cb.feasibility_rates[0] == 1.0  # all G == 0

    def test_metrics_interval_constant(self):
        """_METRICS_INTERVAL should be a positive integer."""
        from src.experiments.ga_experiment import _METRICS_INTERVAL

        assert isinstance(_METRICS_INTERVAL, int)
        assert _METRICS_INTERVAL > 0
