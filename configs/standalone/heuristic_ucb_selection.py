"""
Heuristic Test: ucb_selection (Meta-Heuristic)

Standalone configuration for isolated ucb_selection testing.
...
@dataclass
class UcbSelectionConfig:
    """UCB selection heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-ucb-selection"
    experiment_id: str = "F-META-01"
    environment: str = "test"
    notes: str = "Isolated test: ucb_selection"
    ...
    # === HEURISTIC TOGGLES (Only ucb_selection ON) ===
    ...
    heuristic_ucb_selection: bool = True
...
def get_test_config() -> UcbSelectionConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return UcbSelectionConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> UcbSelectionConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return UcbSelectionConfig(ngen=2000, pop_size=200, environment="prod")
