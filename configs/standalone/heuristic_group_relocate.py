"""
Heuristic Test: group_relocate (Perturbation)

Standalone configuration for isolated group_relocate testing.
...
@dataclass
class GroupRelocateConfig:
    """Group relocate heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-group-relocate"
    experiment_id: str = "F-PERTURB-05"
    environment: str = "test"
    notes: str = "Isolated test: group_relocate"
    ...
    # === HEURISTIC TOGGLES (Only group_relocate ON) ===
    ...
    heuristic_group_relocate: bool = True
...
def get_test_config() -> GroupRelocateConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return GroupRelocateConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> GroupRelocateConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return GroupRelocateConfig(ngen=2000, pop_size=200, environment="prod")
