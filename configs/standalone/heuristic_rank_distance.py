"""
Heuristic Test: rank_distance (Diversity)

Standalone configuration for isolated rank_distance testing.
...
@dataclass
class RankDistanceConfig:
    """Rank distance heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-rank-distance"
    experiment_id: str = "F-DIVERSITY-02"
    environment: str = "test"
    notes: str = "Isolated test: rank_distance"
    ...
    # === HEURISTIC TOGGLES (Only rank_distance ON) ===
    ...
    heuristic_rank_distance: bool = True
...
def get_test_config() -> RankDistanceConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return RankDistanceConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> RankDistanceConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return RankDistanceConfig(ngen=2000, pop_size=200, environment="prod")
