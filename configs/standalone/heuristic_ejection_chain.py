"""
Heuristic Test: ejection_chain (Perturbation)

Standalone configuration for isolated ejection_chain testing.
...
@dataclass
class EjectionChainConfig:
    """Ejection chain heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-ejection-chain"
    experiment_id: str = "F-PERTURB-04"
    environment: str = "test"
    notes: str = "Isolated test: ejection_chain"
    ...
    # === HEURISTIC TOGGLES (Only ejection_chain ON) ===
    ...
    heuristic_ejection_chain: bool = True
...
def get_test_config() -> EjectionChainConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return EjectionChainConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> EjectionChainConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return EjectionChainConfig(ngen=2000, pop_size=200, environment="prod")
