"""
Heuristic Test: epsilon_greedy (Meta-Heuristic)

Standalone configuration for isolated epsilon_greedy testing.
...
class EpsilonGreedyConfig:
    """Epsilon greedy heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-epsilon-greedy"
    experiment_id: str = "F-META-03"
    environment: str = "test"
    notes: str = "Isolated test: epsilon_greedy"
    ...
    # === HEURISTIC TOGGLES (Only epsilon_greedy ON) ===
    ...
    heuristic_epsilon_greedy: bool = True
...
def get_test_config() -> EpsilonGreedyConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return EpsilonGreedyConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> EpsilonGreedyConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return EpsilonGreedyConfig(ngen=2000, pop_size=200, environment="prod")
