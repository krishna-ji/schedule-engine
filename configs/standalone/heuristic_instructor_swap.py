"""
Heuristic Test: instructor_swap (Perturbation)

Standalone configuration for isolated instructor_swap testing.
...
@dataclass
class InstructorSwapConfig:
    """Instructor swap heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-instructor-swap"
    experiment_id: str = "F-PERTURB-01"
    environment: str = "test"
    notes: str = "Isolated test: instructor_swap"
...
def get_test_config() -> InstructorSwapConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return InstructorSwapConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> InstructorSwapConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return InstructorSwapConfig(ngen=2000, pop_size=200, environment="prod")
