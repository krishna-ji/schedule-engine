"""
Heuristic Test: slot_repair (Repair)

Standalone configuration for isolated slot_repair testing.
...
@dataclass
class SlotRepairConfig:
    """Slot repair heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-slot-repair"
    experiment_id: str = "F-REPAIR-01"
    environment: str = "test"
    notes: str = "Isolated test: slot_repair"
    ...
    # === HEURISTIC TOGGLES (Only slot_repair ON) ===
    ...
    heuristic_slot_repair: bool = True
...
def get_test_config() -> SlotRepairConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return SlotRepairConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> SlotRepairConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return SlotRepairConfig(ngen=2000, pop_size=200, environment="prod")
