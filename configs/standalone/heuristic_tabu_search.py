"""
Heuristic Test: tabu_search (Improvement)

Standalone configuration for isolated tabu_search testing.
...
class TabuSearchConfig:
    """Tabu search heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-tabu-search"
    experiment_id: str = "F-IMPROVE-02"
    environment: str = "test"
    notes: str = "Isolated test: tabu_search"
    ...
    # === HEURISTIC TOGGLES (Only tabu_search ON) ===
    ...
    heuristic_tabu_search: bool = True
...
def get_test_config() -> TabuSearchConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return TabuSearchConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> TabuSearchConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return TabuSearchConfig(ngen=2000, pop_size=200, environment="prod")
