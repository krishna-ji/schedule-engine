"""
Heuristic Test: greedy_sequential_assignment (Construction)

Standalone configuration for isolated greedy_sequential_assignment testing.
...
@dataclass
class GreedySequentialConfig:
    """Greedy sequential assignment heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-greedy-sequential"
    experiment_id: str = "F-CONSTRUCT-01"
    environment: str = "test"
    notes: str = "Isolated test: greedy_sequential_assignment"
    output_subdir: str = "f-construction"
    ...
    # === KILLSWITCHES (Only target heuristic ON) ===
    ...
    # === HEURISTIC TOGGLES (Only greedy_sequential ON) ===
    heuristic_greedy_sequential: bool = True
...
def get_test_config() -> GreedySequentialConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return GreedySequentialConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> GreedySequentialConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return GreedySequentialConfig(ngen=2000, pop_size=200, environment="prod")
