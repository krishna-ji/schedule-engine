"""
Heuristic Test: entropy_preserving (Diversity)

Standalone configuration for isolated entropy_preserving testing.
...
@dataclass
class EntropyPreservingConfig:
    """Entropy preserving heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-entropy-preserving"
    experiment_id: str = "F-DIVERSITY-03"
    environment: str = "test"
    notes: str = "Isolated test: entropy_preserving"
    output_subdir: str = "f-diversity"
    ...
    # === HEURISTIC TOGGLES (Only entropy_preserving ON) ===
    ...
    heuristic_entropy_preserving: bool = True
...
def get_test_config() -> EntropyPreservingConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return EntropyPreservingConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> EntropyPreservingConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return EntropyPreservingConfig(ngen=2000, pop_size=200, environment="prod")
