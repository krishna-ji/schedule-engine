"""
Heuristic Test: room_swap (Perturbation)

Standalone configuration for isolated room_swap testing.
...
@dataclass
class RoomSwapConfig:
    """Room swap heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-room-swap"
    experiment_id: str = "F-PERTURB-02"
    environment: str = "test"
    notes: str = "Isolated test: room_swap"
    ...
    # === HEURISTIC TOGGLES (Only room_swap ON) ===
...
def get_test_config() -> RoomSwapConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return RoomSwapConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> RoomSwapConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return RoomSwapConfig(ngen=2000, pop_size=200, environment="prod")
