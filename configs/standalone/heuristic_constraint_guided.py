"""
Heuristic Test: constraint_guided_generation (Construction)

Standalone configuration for isolated constraint_guided_generation testing.
...
@dataclass
class ConstraintGuidedConfig:
    """Constraint guided generation heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-constraint-guided"
    experiment_id: str = "F-CONSTRUCT-02"
    environment: str = "test"
    notes: str = "Isolated test: constraint_guided_generation"
    ...
    # === HEURISTIC TOGGLES (Only constraint_guided ON) ===
    heuristic_greedy_sequential: bool = False
    heuristic_constraint_guided: bool = True
...
def get_test_config() -> ConstraintGuidedConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return ConstraintGuidedConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> ConstraintGuidedConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return ConstraintGuidedConfig(ngen=2000, pop_size=200, environment="prod")
