"""
Heuristic Test: crowding_distance (Diversity)

Standalone configuration for isolated crowding_distance testing.
...
@dataclass
class CrowdingDistanceConfig:
    """Crowding distance heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-crowding-distance"
    experiment_id: str = "F-DIVERSITY-01"
    environment: str = "test"
    notes: str = "Isolated test: crowding_distance"
    ...
    # === HEURISTIC TOGGLES (Only crowding_distance ON) ===
    ...
    heuristic_crowding_distance: bool = True
...
def get_test_config() -> CrowdingDistanceConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return CrowdingDistanceConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> CrowdingDistanceConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return CrowdingDistanceConfig(ngen=2000, pop_size=200, environment="prod")
