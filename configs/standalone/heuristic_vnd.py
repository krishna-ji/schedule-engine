"""
Heuristic Test: vnd (Variable Neighborhood Descent - Improvement)

Standalone configuration for isolated vnd testing.
...
@dataclass
class VndConfig:
    """Variable neighborhood descent heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-vnd"
    experiment_id: str = "F-IMPROVE-04"
    environment: str = "test"
    notes: str = "Isolated test: vnd"
...
def get_test_config() -> VndConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return VndConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> VndConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return VndConfig(ngen=2000, pop_size=200, environment="prod")
