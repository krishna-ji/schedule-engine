"""
Heuristic Test: feasibility_pump (Repair)

Standalone configuration for isolated feasibility_pump testing.
...
@dataclass
class FeasibilityPumpConfig:
    """Feasibility pump heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-feasibility-pump"
    experiment_id: str = "F-REPAIR-02"
    environment: str = "test"
    notes: str = "Isolated test: feasibility_pump"
    ...
    # === HEURISTIC TOGGLES (Only feasibility_pump ON) ===
    ...
    heuristic_feasibility_pump: bool = True
...
def get_test_config() -> FeasibilityPumpConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return FeasibilityPumpConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> FeasibilityPumpConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return FeasibilityPumpConfig(ngen=2000, pop_size=200, environment="prod")
