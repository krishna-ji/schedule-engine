"""
Heuristic Test: hillclimb (Improvement)

Standalone configuration for isolated hillclimb testing.
...
class HillclimbConfig:
    """Hillclimb heuristic test config."""

    # === METADATA ===
    name: str = "heuristic-hillclimb"
    experiment_id: str = "F-IMPROVE-01"
    environment: str = "test"
    notes: str = "Isolated test: hillclimb"
    ...
    # === HEURISTIC TOGGLES (Only hillclimb ON) ===
...
def get_test_config() -> HillclimbConfig:
    """Get test profile config (30 gens, 10 pop)."""
    return HillclimbConfig(ngen=30, pop_size=10, environment="test")


def get_prod_config() -> HillclimbConfig:
    """Get production profile config (2000 gens, 200 pop)."""
    return HillclimbConfig(ngen=2000, pop_size=200, environment="prod")
